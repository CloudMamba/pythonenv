...
def create_encrypt_button(master, key_entry, message_entry, result_label):
    create_button(master, "Encrypt", lambda: result_label.config(text=f"Encrypted message: {aes_encrypt(key_entry.get().encode('utf-8'), message_entry.get().encode('utf-8')).hex()}"), 2, 0)
